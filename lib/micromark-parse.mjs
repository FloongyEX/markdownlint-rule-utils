...
/*
 * micromark-core-commonmark must exactly match
 * what's used by micromark for the shim below to work correctly
 * Unfortunately, omitting this dependency from package.json
 * breaks strict dependency resolution (e.g., pnpm)
 */
import { labelEnd } from "micromark-core-commonmark"
import { isHtmlFlowComment } from "markdownlint-rule-helpers/micromark"
...
export function getEvents(
  ...
  function tokenizeShim(effects, okOriginal, nokOriginal) {
    ...
    const nokShim = (code) => {
      ...
      if (indexStart >= 0) {
        ...
        const previousUndefinedEvent = artificialEventLists.length > 0
          && artificialEventLists[artificialEventLists.length - 1][0]
        const previousUndefinedToken = previousUndefinedEvent
          && previousUndefinedEvent[1]

        if (
          previousUndefinedToken
          && previousUndefinedToken.end.line
          === undefinedReferenceType.start.line
          && previousUndefinedToken.end.column
          === undefinedReferenceType.start.column
        ) {
  ...
  try {
    ...
    const eol = true
    const parseContext = micromarkParse({
      ...micromarkParseOptions,
      extensions
    })
    ...
    if (kind === "enter" && !skipHtmlFlowChildren) {
      ...
      if (current.type === "htmlFlow" && !isHtmlFlowComment(current)) {
        ...
        if (!reparseOptions || !lines) {
          reparseOptions = {
            ...micromarkParseOptions,
            extensions: [{ disable: { null: ["codeIndented", "htmlFlow"] } }],
          }
